  ...
  return (
    <div className={`flex items-start gap-3 bg-gradient-to-b from-red-50 to-rose-50/80 border border-red-200 rounded-xl p-4 ${className}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] uppercase tracking-wide font-extrabold text-red-500">{getUiText('actionRequired', language)}</p>
        <p className="text-sm font-bold text-red-800 mt-0.5">{getUiText('somethingWentWrong', language)}</p>
      ...
      {onRetry && (
        ...
        >
          <RefreshCw size={12} /> {getUiText('retry', language)}
        </button>
      )}
    </div>
  );
